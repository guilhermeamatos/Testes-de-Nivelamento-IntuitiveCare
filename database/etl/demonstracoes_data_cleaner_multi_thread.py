...
def process_file(file, registro_ans_set, columns_to_replace_comma):
...
def process_files_subset(files_subset, registro_ans_set, columns_to_replace_comma):
    for file in files_subset:
        process_file(file, registro_ans_set, columns_to_replace_comma)

def process_selected_files_multithread_4threads(input_dir, operadora_file, files_to_process, columns_to_replace_comma):
    ...
    file_groups = [files_to_process[i:i+2] for i in range(0, len(files_to_process), 2)]

    threads = []
    for group in file_groups:
        thread = threading.Thread(target=process_files_subset, args=(group, registro_ans_set, columns_to_replace_comma))
        thread.start()
...
process_selected_files_multithread_4threads(input_dir, operadora_file, files_to_process, columns_to_replace_comma)
